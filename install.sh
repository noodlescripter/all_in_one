...
if [[ "$SHELL" == "/bin/zsh" ]]; then
    ...
    echo 'export JAVA_HOME=~/sSoftware/all_in_one/jdk-11.0.18.jdk/Contents/Home' >> ~/.zshrc
...
# Check if the user is using bash
    ...
    if [ -f "$HOME/.bashrc" ]; then
        echo 'export JAVA_HOME=~/sSoftware/all_in_one/jdk-11.0.18.jdk/Contents/Home' >> ~/.bashrc
